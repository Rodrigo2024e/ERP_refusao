...
public class ReceiptItemService {
    ...
    public ReceiptItemDTO findById(Long receiptId, Long partnerId, Long materialId) {
        ...
        ReceiptItem entity = obj.orElseThrow(() -> new ResourceNotFoundException("ReceiptItem not found for the given IDs"));
    ...
    public ReceiptItemDTO insert(Long parentReceiptId, ReceiptItemDTO dto) {
        ...
        if (dto.getReceiptId() != null && !dto.getReceiptId().equals(parentReceiptId)) {
            throw new IllegalArgumentException("O Receipt ID no corpo (" + dto.getReceiptId() + ") não corresponde ao ID do Path (" + parentReceiptId + ")");
        }
        ...
        ReceiptItemPK pk = buildReceiptItemPK(
            parentReceiptId, 
            dto.getPartnerId(), 
            dto.getMaterialId()
        );
